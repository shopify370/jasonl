...
class Ion_auth_model extends CI_Model
{
	...
	public function hash_password_db_ws($id, $password, $use_sha1_override=FALSE)
	{
		if (empty($id) || empty($password))
		{
			return FALSE;
		...
		$query = $this->db->select('password, salt')
		                  ->where("staff_id  = '$id' and is_deleted = '0'")
		                  ->limit(1)
		                  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
	...
	public function reset_password($identity, $new) {
		...
		if($this->db->update($this->tables['users'], $data, array($this->identity_column => $identity,'is_deleted' => '0'))){
			// echo $this->db->last_query();die;
			$return = 1;
		} 
	...
	public function email_check($email = '')
	{
		...
		return $this->db->where("email = '$email' and is_deleted = '0'")
										->group_by("id")
										->order_by("id", "ASC")
										->limit(1)
		                ->count_all_results($this->tables['users']) > 0;
...
//			}
		...
		// $password   = $this->hash_password_db_send($additional_data['staff_id'],$password);
		// debug ($additional_data['staff_id']);die;
		// debug($password);die;
		// Users table.
		$data = array(
		    'username'   => $username,

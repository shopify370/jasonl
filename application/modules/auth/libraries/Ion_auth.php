...
class Ion_auth
{
	...
	public function forgotten_password($identity)    //changed $email to $identity
	{
      ...
      $user = $this->where($identifier, $identity)->where('active', 1)->users()->row();  //changed to get_user_by_identity from email
			...
			{
				$data = array(
					'identity'		=> $user->{$this->config->item('first_name', 'ion_auth')},
...
//debug($additional_data);
	...
	public function is_admin($id=false)
	{
      ...
      public function is_staff_admin_1($id = false) {
          
        $this->ion_auth_model->trigger_events('is_staff_admin_1');

        $admin_group = $this->config->item('staff_admin_1_group', 'ion_auth');
        
        return $this->in_group($admin_group, $id);
    }
    
      public function is_staff_admin_2($id = false) {
          
        $this->ion_auth_model->trigger_events('is_staff_admin_2');

        $admin_group = $this->config->item('staff_admin_2_group', 'ion_auth');
